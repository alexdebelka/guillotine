...
def query_pool_map(manifest: dict) -> dict:
    """{query_id: (dataset, split)} for every submission query."""
    m = {}
    for key, pool in manifest.items():
        if key == "train_pairs":
            continue
...
def fuse_branches(branch_dfs: dict, manifest: dict, weights_by_dataset=None,
                  k=RRF_K) -> dict:
    """Per-dataset RRF over multiple branches → pool_rankings for write_submission.

    branch_dfs: {branch_name: DataFrame[query_id, target_id, score]} spanning any pools.
    weights_by_dataset: {dataset: {branch_name: weight}}; missing → weight 1.0.
        e.g. lean spatial on d1, invariant on d2/d3:
        {'dataset1': {'baseline': 1.0, 'b2': 0.2},
         'dataset2': {'baseline': 0.3, 'b2': 1.0},
         'dataset3': {'baseline': 0.3, 'b2': 1.0}}
    Returns {(dataset, split): {query_id: [target_id full ranking]}}.
    """
    weights_by_dataset = weights_by_dataset or {}
    q2pool = query_pool_map(manifest)
    # per-branch rankings, then split by pool
    ...
    for key, pool in manifest.items():
        ...
        names = list(branch_dfs)
        w = weights_by_dataset.get(ds, {})
        branch_list, weight_list = [], []
        for name in names:
            r = {q: rankings[name].get(q, []) for q in pool.query_ids}
